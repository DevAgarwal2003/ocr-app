...
# Load the translation model directly from Hugging Face
@st.cache_resource
def load_translation_model(model_name="Helsinki-NLP/opus-mt-en-hi"):
    # Use the tf.device context to ensure model loads on CPU
    with tf.device('/CPU:0'):
        tokenizer = AutoTokenizer.from_pretrained("Helsinki-NLP/opus-mt-en-hi", use_fast=True)
        translation_model = TFAutoModelForSeq2SeqLM.from_pretrained(model_name)
    return tokenizer, translation_model

# Load the model using the Hugging Face model name
